...
public class vp_FPCamera : vp_Component
{
	...
	protected override void LateUpdate()
	{
		...
		Transform.localEulerAngles +=
			vp_MathUtility.NaNSafeVector3(Vector3.forward * (m_RotationSpring.State.z + currentCamRot));
    ...
    float currentCamRot = 0;
    public float camRotSpeed = 10;
    public bool isWallrunAllowed = true;

    private void UpdateWallRunAnimation()
    {
        if (isWallrunAllowed)
            currentCamRot = Mathf.Lerp(currentCamRot, wallRunning.targetCamRot, camRotSpeed * Time.deltaTime);

        //Quaternion rot = cam.localRotation;
        //cam.localRotation = Quaternion.Euler(0, 0, currentCamRot);
    }
