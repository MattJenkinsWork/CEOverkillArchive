...
public class vp_FPController : vp_CharacterController
{
    ...
    public float MotorWallJumpForce = 0f; //MAX WALL JUMP FORCE
    ...
    public bool CanDoubleJump = true; //MAX - Variable stating whether double jump is available
    ...
    bool m_DidFirstJump = false; //MAX - Bool to state whether first jump has been acted upon
    public bool m_DidSecondJump = false; //MAX - Bool to check if second jump has been done and is or isn't available
	...
	protected override void Start()
	{
        ...
        wallrunController = this.GetComponent<WallRunning>();
	...
	protected override void Update()
	{
        ...
        //Debug.Log(hasWallJump.ToString() + CanDoubleJump.ToString() + m_DidFirstJump.ToString() + m_DidSecondJump.ToString());
	...
	protected override void FixedMove()
	{
		...
		{
            Debug.Log("UFPS thinks the player is dead");
	...
	protected virtual bool CanStart_Jump()
	{
        ...
        //MAX - Checks if the character has wallrun to reset jumps
        if (wallrunController.jumpReset)
        {
            //Debug.Log("Wallrun JumpReset Checked");
            return true;
        }


        //MAX - Using the UFPS framework, checks if double jump is available
        if (CanDoubleJump)
        {
            //if (m_DidFirstJump && m_DidSecondJump && wallrunController.jumpReset == false)
            //    return false;

            if (hasWallJump)
                m_DidFirstJump = true;
                hasWallJump = false;
            if (m_DidFirstJump && m_DidSecondJump)
                return false;
            else if (m_DidFirstJump && !m_DidSecondJump)
                return true;
	...
	protected virtual void OnStart_Jump()
	{
        ...
        // MAX
        if (wallrunController.jumpReset && !hasWallJump)
        {
            m_MotorJumpDone = false;
            m_DidFirstJump = false;
            m_DidSecondJump = false;
            wallrunController.jumpReset = false;
        }

        if (wallrunController.IsWallrunning)
        {
            m_MotorThrottle += wallrunController.normal * MotorWallJumpForce * Time.timeScale;
            hasWallJump = true;
        }
        ...
        if (CanDoubleJump)
        {
            ...
            {
                m_DidSecondJump = true;
                wallrunController.jumpReset = false;
                //Debug.Log("Jump Reset false");
            }
        }
        ...
        if (CanDoubleJump)
        {
            ...
            {
                m_MotorThrottle.y = ((MotorJumpForce * 4) / Time.timeScale);
            }
        }
        else
        {
